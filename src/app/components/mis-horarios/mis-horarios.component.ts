...
export class MisHorariosComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.listaEspecialidades = Object.values(
      this.inputEspecialista.especialidad
    );
  }
  ...
  seleccionarEspecialidad(especialidad: any) {
    ...
    this.formHorarios.controls['especialidad'].setValue(
      this.especialidadSeleccionada
    );

    this.db
      .getDisponibilidadesByEspecialistaEspecialidad(
        this.inputEspecialista.dni,
        this.especialidadSeleccionada
      )
      .subscribe((ref: any) => {
        if (ref.length != '0') {
  ...
  altaHorarios() {
    ...
    if (this.listadoHorariosEspecialidad != null) {
      this.db.baja(
        this.listadoHorariosEspecialidad.id,
    ...
    let horariosEspecialidad: DiponibilidadHorariaEspecialidad =
      new DiponibilidadHorariaEspecialidad(
        this.listaDiasEspecialidad,
        this.inputEspecialista,
        especialidad,
        duracionTurno
      );
    ...
    this.db
      .alta(
        JSON.parse(JSON.stringify(horariosEspecialidad)),
        'horarios_especialidad'
      )
